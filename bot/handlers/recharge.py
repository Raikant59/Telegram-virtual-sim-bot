# bot/handlers/recharge.py
import re, requests
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton, ForceReply
from models.user import User
from models.recharge import Recharge
from models.payment_config import PaymentConfig
from models.transaction import Transaction
from bot.libs.helpers import safe_edit_message, is_admin

AMOUNTS = [100, 200, 500]


def _cfg():
    return PaymentConfig.objects().first()


def _ensure_user(bot, from_):
    uid = str(from_["id"])
    user = User.objects(telegram_id=uid).first()
    if not user:
        user = User(telegram_id=uid, username=from_.get("username", "")).save()
    return user


# Entry point: user taps "Recharge"
def menu(bot, call):
    user = _ensure_user(bot, call["from"])
    cfg = _cfg()
    if not cfg:
        bot.answer_callback_query(call["id"], "Payments not configured yet.")
        return

    text = "💳 <b>Recharge</b>\nChoose amount below."
    markup = amount_keyboard(cfg)
    safe_edit_message(bot, call, text, markup)
    bot.answer_callback_query(call["id"])


def amount_keyboard(cfg):
    kb = InlineKeyboardMarkup(row_width=3)
    btns = [InlineKeyboardButton(f"{a} 💎", callback_data=f"amt:{a}") for a in AMOUNTS]
    kb.row(*btns)
    kb.row(InlineKeyboardButton("Enter custom amount", callback_data="amt:custom"))
    kb.row(InlineKeyboardButton("« Back", callback_data="back_main"))
    return kb


def amount_callback(bot, call):
    # data: "amt:<value|custom>"
    _, v = call["data"].split(":")
    cfg = _cfg()

    if v == "custom":
        bot.answer_callback_query(call["id"], "Send the amount (numbers only).")
        bot.send_message(
            call["message"]["chat"]["id"],
            f"💵 Enter amount (>= {cfg.min_amount:.0f} 💎).",
            reply_markup=ForceReply(selective=True),
        )
        return

    amount = float(v)
    if amount < (cfg.min_amount or 0):
        bot.answer_callback_query(call["id"], f"Min amount is {cfg.min_amount:.0f} 💎")
        return

    _ask_method(bot, call, amount)


def capture_custom_amount(bot, message):
    reply = message.get("reply_to_message")
    if not reply:
        return
    if not reply.get("text", "").startswith("💵 Enter amount"):
        return

    txt = (message.get("text") or "").strip()
    try:
        amount = float(txt)
    except ValueError:
        bot.send_message(message["chat"]["id"], "⚠️ Invalid amount.")
        return

    cfg = _cfg()
    if amount < (cfg.min_amount or 0):
        bot.send_message(
            message["chat"]["id"], f"❌ Min amount is {cfg.min_amount:.2f} 💎"
        )
        return

    _ask_method(bot, message, amount)


def _ask_method(bot, source, amount):
    cfg = _cfg()
    text = f"💵 Amount chosen: {amount:.2f} 💎\n\nSelect payment method:"
    kb = InlineKeyboardMarkup(row_width=1)

    if cfg.enable_manual:
        kb.add(
            InlineKeyboardButton(
                "🇮🇳 Manual (UPI/QR)", callback_data=f"pay:manual:{amount}"
            )
        )
    if cfg.enable_bharatpay:
        kb.add(
            InlineKeyboardButton(
                "🏦 BharatPay", callback_data=f"pay:bharatpay:{amount}"
            )
        )
    if cfg.enable_crypto:
        kb.add(
            InlineKeyboardButton(
                "🪙 Crypto", callback_data=f"pay:crypto:{amount}"
            )
        )
    kb.add(InlineKeyboardButton("« Back", callback_data="recharge"))

    if "message" in source:  # coming from callback
        safe_edit_message(bot, source, text, kb)
    else:  # coming from message reply
        bot.send_message(
            source["chat"]["id"], text, reply_markup=kb, parse_mode="HTML"
        )


def pay_callback(bot, call):
    # data: "pay:<method>:<amount>"
    _, method, amount_str = call["data"].split(":")
    amount = float(amount_str)
    cfg = _cfg()

    if method == "manual":
        disp = (
            "🇮🇳 <b>Manual UPI Recharge</b>\n"
            f"Amount: {amount:.2f} 💎\nUPI: <code>{cfg.manual_upi_id or '-'}</code>\n"
            f"{'📷 QR: ' + cfg.manual_qr_url if cfg.manual_qr_url else ''}\n\n"
            "After paying, tap 'I Paid • Submit UTR' and reply with your UTR."
        )
        kb = InlineKeyboardMarkup()
        kb.add(
            InlineKeyboardButton(
                "✅ I Paid • Submit UTR", callback_data=f"utr:{amount}:{method}"
            )
        )
        kb.add(InlineKeyboardButton("« Back", callback_data="recharge"))
        safe_edit_message(bot, call, disp, kb)

    elif method in ["crypto", "bharatpay"]:
        disp = (
            f"🔗 <b>{method.capitalize()} Recharge</b>\n"
            f"Amount: {amount:.2f} 💎\nTap below to generate link."
        )
        kb = InlineKeyboardMarkup()
        kb.add(
            InlineKeyboardButton(
                "🔗 Generate Link", callback_data=f"utr:{amount}:{method}"
            )
        )
        kb.add(InlineKeyboardButton("« Back", callback_data="recharge"))
        safe_edit_message(bot, call, disp, kb)

    bot.answer_callback_query(call["id"])


def ask_utr_callback(bot, call):
    # data: "utr:<amount>:<method>"
    _, amt, method = call["data"].split(":")
    amount = float(amt)
    chat_id = call["message"]["chat"]["id"]
    user = _ensure_user(bot, call["from"])

    if method == "manual":
        r = Recharge(
            user=user,
            method="manual",
            amount=amount,
            currency="INR",
            status="awaiting_utr",
            chat_id=chat_id,
        ).save()
        msg = bot.send_message(
            chat_id,
            "📮 Reply to this message with your <b>UTR</b> number.",
            parse_mode="HTML",
            reply_markup=ForceReply(selective=True),
        )
        r.request_message_id = msg.message_id
        r.save()

    else:  # crypto / bharatpay
        cfg = _cfg()
        r = Recharge(
            user=user,
            method=method,
            amount=amount,
            currency="INR",
            status="pending",
            chat_id=chat_id,
        ).save()
        try:
            if method == "crypto":
                payload = {
                    "amount": amount,
                    "currency": "INR",
                    "account": cfg.crypto_wallet_or_account,
                    "reference": str(r.id),
                    "callback": f"/webhooks/crypto",
                }
                headers = (
                    {"Authorization": f"Bearer {cfg.crypto_api_key}"}
                    if cfg.crypto_api_key
                    else {}
                )
                resp = requests.post(
                    cfg.crypto_create_invoice_url,
                    json=payload,
                    headers=headers,
                    timeout=10,
                )
                data = (
                    resp.json()
                    if resp.headers.get("content-type", "").startswith("application/json")
                    else {}
                )
                r.provider_txn_id = data.get("id") or data.get("invoice_id")
                r.payment_link = data.get("hosted_url") or data.get("invoice_url")
                r.address_or_upi = data.get("address")
                r.details = data
                r.save()
            else:
                payload = {
                    "merchant_id": cfg.bharatpay_merchant_id,
                    "amount": amount,
                    "reference": str(r.id),
                    "callback": f"/webhooks/bharatpay",
                }
                headers = (
                    {"X-API-KEY": cfg.bharatpay_api_key}
                    if cfg.bharatpay_api_key
                    else {}
                )
                resp = requests.post(
                    cfg.bharatpay_create_order_url,
                    json=payload,
                    headers=headers,
                    timeout=10,
                )
                data = (
                    resp.json()
                    if resp.headers.get("content-type", "").startswith("application/json")
                    else {}
                )
                r.provider_txn_id = data.get("order_id") or data.get("id")
                r.payment_link = data.get("payment_url") or data.get("short_url")
                r.address_or_upi = cfg.bharatpay_upi_id
                r.details = data
                r.save()

            if r.payment_link:
                kb = InlineKeyboardMarkup()
                kb.add(InlineKeyboardButton("🔗 Pay Now", url=r.payment_link))
                kb.add(InlineKeyboardButton("« Back", callback_data="recharge"))
                safe_edit_message(
                    bot,
                    call,
                    f"✅ Link generated.\nAmount: {amount:.2f} 💎\nOpen the link to complete payment.",
                    kb,
                )
            else:
                safe_edit_message(
                    bot,
                    call,
                    "⚠️ Failed to generate a payment link. Try another method.",
                    None,
                )
                r.mark("failed")
        except Exception as e:
            r.mark("failed", details={"error": str(e)})
            safe_edit_message(bot, call, f"⚠️ Error: {e}", None)

    bot.answer_callback_query(call["id"])


# Capture UTR replies
def capture_utr_message(bot, message):
    reply = message.get("reply_to_message")
    if not reply:
        return
    r = Recharge.objects(
        chat_id=message["chat"]["id"],
        request_message_id=reply["message_id"],
        status="awaiting_utr",
    ).first()
    if not r:
        return
    utr = (message.get("text") or "").strip()
    if not re.match(r"^[A-Za-z0-9\-]{6,25}$", utr):
        bot.send_message(
            message["chat"]["id"],
            "❌ Invalid UTR format. Please send only the UTR (6–25 chars).",
        )
        return
    r.utr = utr
    r.mark("pending")
    bot.send_message(
        message["chat"]["id"],
        f"✅ UTR received: <code>{utr}</code>\nWe’ll verify shortly.",
        parse_mode="HTML",
    )
    _notify_admins(bot, r)


def _notify_admins(bot, r: Recharge):
    from models.admin import Admin

    admins = Admin.objects()
    if not admins:
        return
    text = (
        "🆕 <b>Recharge Pending</b>\n\n"
        f"User: <code>{r.user.telegram_id}</code>\n"
        f"Method: {r.method}\nAmount: {r.amount:.2f} 💎\n"
        f"UTR/Txn: <code>{r.utr or r.provider_txn_id or '-'}</code>\n"
        f"ID: {r.id}\n\nApprove?"
    )
    kb = InlineKeyboardMarkup()
    kb.add(
        InlineKeyboardButton("✅ Approve", callback_data=f"rcg:approve:{r.id}"),
        InlineKeyboardButton("❌ Reject", callback_data=f"rcg:reject:{r.id}"),
    )
    for a in admins:
        try:
            bot.send_message(
                int(a.telegram_id), text, parse_mode="HTML", reply_markup=kb
            )
        except Exception:
            pass


def action_callback(bot, call):
    if not is_admin(str(call["from"]["id"])):
        bot.answer_callback_query(call["id"], "Not allowed.")
        return

    try:
        _, action, rid = call["data"].split(":")
    except ValueError:
        bot.answer_callback_query(call["id"], "Invalid action.")
        return

    r = Recharge.objects(id=rid).first()
    if not r:
        bot.edit_message_text(
            "Recharge not found.",
            chat_id=call["message"]["chat"]["id"],
            message_id=call["message"]["message_id"],
        )
        return

    user = r.user
    if not user:
        bot.edit_message_text(
            "User not found.",
            chat_id=call["message"]["chat"]["id"],
            message_id=call["message"]["message_id"],
        )
        return

    details_text = (
        f"User: {user.telegram_id}\n"
        f"Method: {r.method}\n"
        f"Amount: {r.amount:.2f} 💎\n"
        f"UTR/Txn: {r.utr or r.provider_txn_id or 'N/A'}"
    )

    if action == "approve" and r.status in ["pending", "awaiting_utr"]:
        user.balance += r.amount
        user.total_recharged = (user.total_recharged or 0) + r.amount
        user.save()

        Transaction(
            user=user,
            type="credit",
            amount=r.amount,
            closing_balance=user.balance,
            note=f"recharge via {r.method}",
        ).save()

        r.mark("paid")

        bot.edit_message_text(
            f"Recharge Approved ✅\n\n{details_text}",
            chat_id=call["message"]["chat"]["id"],
            message_id=call["message"]["message_id"],
        )
        bot.send_message(
            user.telegram_id,
            f"✅ Your recharge of {r.amount:.2f} 💎 via {r.method} has been approved.\n"
            f"New Balance: {user.balance:.2f} 💎",
        )

    elif action == "reject" and r.status in ["pending", "awaiting_utr"]:
        r.mark("rejected")

        bot.edit_message_text(
            f"Recharge Rejected ❌\n\n{details_text}",
            chat_id=call["message"]["chat"]["id"],
            message_id=call["message"]["message_id"],
        )
        bot.send_message(
            user.chat_id,
            f"❌ Your recharge of {r.amount:.2f} 💎 via {r.method} has been rejected by admin.\n"
            f"If this is a mistake, please contact support.",
        )

    else:
        bot.edit_message_text(
            f"Nothing to do.\n\n{details_text}",
            chat_id=call["message"]["chat"]["id"],
            message_id=call["message"]["message_id"],
        )
